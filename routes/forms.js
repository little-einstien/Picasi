...
router.post('/:pid', function (req, res) {
    ...
    saveFlow({pid:data.pid},data).then((result) => {
...
//            params.status = 1;
        ...
        mongodb.get().db(config.mongo.db).collection(config.mongo.forms_col).find(params).toArray(function (err, result) {
            console.log(result);
            if (err) {
                console.log(err);
...
function saveFlow(where,data) {
    console.log("Going to save form");
    return new Promise(function (resolve, reject) {
        let db = mongodb.get().db(config.mongo.db);
        //Find the first document in the customers collection:
        db.collection(config.mongo.forms_col).update({pid : -1}, data, { upsert: true, safe: true }, function (err, result) {
            if (err) {
                console.log(err)
                reject({ status: FAILURE });
            }
